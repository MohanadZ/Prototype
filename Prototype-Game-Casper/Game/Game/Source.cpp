...
int main(int argc, char** argv) {
	...
	avatarT.loadFromFile("WizardIdle.png");
	monsterT.loadFromFile("MonsterSheet.png");

	SpriteAnimation avatarAnimation(&avatarT, Vector2u(20, 1), 0.05f);
	...
	while (gameWindow.isOpen()) {
		...
		if (idle == 1) {
			...
			monsterAnimation.updateAnimation(1, deltaTime);
			for (int i = 0; i < sizeof(monster) / sizeof(monster[0]); i++) {
				monster[i].monsterSprite.setTextureRect(monsterAnimation.textureRect);
			}
		}
		if (attack == 1) {
			//Update the animation of the monster sprite
			monsterAnimation.updateAnimation(2, deltaTime);
			for (int i = 0; i < sizeof(monster) / sizeof(monster[0]); i++) {
				monster[i].monsterSprite.setTextureRect(monsterAnimation.textureRect);
			}
		}
		if (dying == 1) {
			//Update the animation of the monster sprite
			monsterAnimation.updateAnimation(3, deltaTime);
			for (int i = 0; i < sizeof(monster) / sizeof(monster[0]); i++) {
				monster[i].monsterSprite.setTextureRect(monsterAnimation.textureRect);
			}
		}
		...
		for (int i = 0; i < monsterNumber; i++) {
			...
			if (monster[i].monsterSprite.getGlobalBounds().intersects(wizard.decoyAvatarSprite.getGlobalBounds())) {
				...
				if (attack == 1 && monsterAnimation.currentImage.x == 19) {
					dying = 1;
					attack = 0;
					monsterAnimation.currentImage.x = 0;
				...
				if (dying == 1 && monsterAnimation.currentImage.x == 19) {
					dying = 0;
					idle = 1;
					setDifficulty();
